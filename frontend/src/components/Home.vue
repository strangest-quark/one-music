   ...
   <template v-if="this.user">
      ...
      <div class="col-sm-6 spotify-column-res">
        <div class="res-content">
          ...
          <button v-on:click="playlists()" class="btn-lg btn-primary btn-dark">Link Playlists</button>
          <!--Playlists checklist-->
          <template v-if="this.playlists">
            <div v-for="playlist in this.playlists" v-bind:key="playlist.id">    
              <input type="checkbox" :id=playlist.id :value=playlist.name v-model="checkedNames"/>
            </div>
          </template>
        </div>
      </div>
    ...
    <template v-else>
      <div class="row">
        <div class="col-sm-6 spotify-column">
          ...
          <!--a href="http://localhost:5000/spotify/"-->
...
export default {
  ...
  computed: {
        ...
        methods: {
            unlink() {
                Vue.axios.get('https://spotify.com/logout')
                this.$store.commit('mutateUser', null);
                this.$router.push({ name: 'Home'})
            },
            playlists(){
                  Vue.axios.get('https://rx00516zgh.execute-api.us-west-2.amazonaws.com/dev/spotify/playlists/',{   
                                params: {
